...
export const getTodos = async (req, res) => {
  try {
    ...
    if (completed !== undefined) {
      results = await req.mongoConn.query("SELECT * FROM todos WHERE completed = ?;", isCompleted);
    } else {
      results = await req.mongoConn.query("SELECT * FROM todos;");
    }
...
export const getTodo = async (req, res) => {
  try {
    ...
    const results = await req.mongoConn.query("SELECT * FROM todos WHERE id = ?", [intId]);
...
export const updateTodo = async (req, res) => {
  try {
    ...
    if (!todo) {
      ...
      await req.mongoConn.query(updateQuery, sqlQueryParams);
      const updatedResults = await req.mongoConn.query("SELECT * FROM todos WHERE id = ?", [intId]);
...
export const deleteTodo = async (req, res) => {
  try {
    ...
    if (!todo) {
      ...
      const resultsDel = await req.mongoConn.query("DELETE FROM todos WHERE id = ?", [intId]);
...
export const createTodo = async (req, res) => {
  try {
    ...
    const result = await req.mongoConn.query(
    ...
    const todo = await req.mongoConn.query("SELECT * FROM todos WHERE id = ?", [result[0].insertId]);
